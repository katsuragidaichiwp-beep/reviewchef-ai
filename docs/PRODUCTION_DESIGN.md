...
現場者画面には、分析、MEOスコア、売上指標、店舗横断データを出さない。ブラウザ上の `app.html?view=staff` は検証用プレビューであり、本番の現場者画面はLINE/SlackのメッセージUIとする。
...
- 完全自動返信は店舗ごとに選択した星数の未返信レビューを全件対象にする
- 対象レビューごとにAIで本文に合った返信を生成し、ユーザー承認なしで指定時刻にGoogleへ投稿する
- 対象星数でも、要チェックキーワードや重大リスク語に該当する場合は `needs_check` に止める
...
8. 完全自動返信が有効な店舗では、選択星数に該当する未返信レビューを `auto_scheduled` にする
9. 要チェックキーワードや重大リスク語に該当するレビューは `needs_check` に止め、管理者確認へ回す
10. 指定時刻のCronで `auto_scheduled` を全件処理し、AI返信を生成または再生成してGoogle Business Profile APIへ投稿
11. `review_replies.posted_at` に返信日時、Google側レスポンス、失敗時のエラーを保存
12. 監査ログと返信済み状態を保存
...
- `status`: 未対応、返信案あり、完全自動返信予定、要チェック、承認待ち、返信済み
...
- `POST /api/reviews/auto-reply/run`
